...
bool atn::Sudoku<T>::propogate_block(atn::Pos p, uint8_t val) {
  ...
  for (uint8_t y = 0; y < T; ++y) {
    for (uint8_t x = 0; x < T; ++x) {
      ...
      if (this->board[block_x * T + x][block_y * T + y].invalid()) return false;
    }
  }
  return true;
}
...
bool atn::Sudoku<T>::propogate(atn::Pos p, uint8_t val) {
  for (uint8_t i = 0; i < T * T; ++i) {
    ...
    if (this->board[p.x][i].invalid()) return false;
    this->board[i][p.y].options.reset(val - 1);
    if (this->board[i][p.y].invalid()) return false;
  }
...
bool atn::Sudoku<T>::validate_row() const {
  ...
  for (uint8_t y = 0; y < T * T; ++y) {
    ...
    for (uint8_t i = 0; i < T * T; ++i) {
      if (!row_contains[i] && !row_options[i]) return false;
    }
  }
  return true;
}
...
bool atn::Sudoku<T>::validate_column() const {
  ...
  for (uint8_t x = 0; x < T * T; ++x) {
    ...
    for (uint8_t i = 0; i < T * T; ++i) {
      if (!column_contains[i] && !column_options[i]) return false;
    }
  }
  return true;
}
    ...
    for (uint8_t y = 0; y < T; ++y) {
      for (uint8_t x = 0; x < T; ++x) {
        if (this->board[block_x * T + x][block_y * T + y].value != atn::UNSET) {
          block_contains.set(
              this->board[block_x * T + x][block_y * T + y].value - 1);
        } else {
          block_options |=
              this->board[block_x * T + x][block_y * T + y].options;
        }
      }
    }
  }
...
void atn::Sudoku<T>::unset(std::vector<atn::Cell<T>> cells) {
  for (uint8_t i = 0; i < cells.size(); ++i) this->unset(cells[i].pos);
}
